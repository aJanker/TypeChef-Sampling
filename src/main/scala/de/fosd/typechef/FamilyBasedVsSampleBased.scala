 ...
 *
 */
object FamilyBasedVsSampleBased extends EnforceTreeHelper with ASTNavigation with ConditionalNavigation with CFGHelper {
  type Task = (String, List[SimpleConfiguration])
  ...
  def checkErrorsAgainstProducts(fm_scanner: FeatureModel, fm: FeatureModel, ast: TranslationUnit, opt: FamilyBasedVsSampleBasedOptions,
                                 logMessage: String) = {
    val (log, fileID, samplingTasks) = initSampling(fm_scanner, fm, ast, opt, logMessage)
    val samplingTasksWithoutFamily = samplingTasks.filterNot {
      _._1 equalsIgnoreCase "family"
    }
    analyzeSATasks(samplingTasksWithoutFamily, ast, fm, opt, fileID, startLog = log)
  }
  ...
  private def analyzeSATasks(tasks: List[Task], tunit: TranslationUnit, fm: FeatureModel, opt: FamilyBasedVsSampleBasedOptions,
                             fileID: String, startLog: String = ""): Unit = {
    println("#start static product analysis")

    if (tasks.size > 0) println("#start task - checking (" + tasks.size + " tasks)")

    tasks.foreach(task => {
      val (taskDesc: String, configs: List[SimpleConfiguration]) = task
      ...
      val results = configs.zipWithIndex.map {
        case (config, current_config) => {
          ...
          val numberOfSelectedFeatures = selectedFeatures.size

          // results (taskName, (NumConfigs, NumASTElems, productDerivationTimes), (warnings, timings, singleWarnings))
          (taskDesc, (numberOfSelectedFeatures, numberOfASTElements, productDerivationTime, product), run)
        }
      }
    ...
    def writeResult(fileID: String, task: (String, List[SimpleConfiguration]), results: List[(String, (Int, Long, Long, TranslationUnit), (List[TypeChefError], List[(String, Long)], Map[String, List[TypeChefError]]))], sum: Boolean = true): Unit = {
      ...
      def writeResults(writer: Writer): Writer = {
        writer.write("[FILE]\t" + fileID + "\n")
        writer.write("[TASK]\t" + taskDesc + "\n")
        writer.write("[CONFIGS]\t" + configs.size + "\n")

        val timingSumresults = results.flatMap(_._3._2).foldLeft(new HashMap[String, Long]) {
          case (map, (analysis, time)) =>
            if (map.contains(analysis)) map + (analysis -> (map.get(analysis).get + time))
            else map + (analysis -> time)
        }.toList
        ...
        results.zipWithIndex.foreach {
          case (result, current_config) => {
            val (taskDesc, (numberOfSelectedFeatures, numberOfASTElements, productDerivationTime, tunit), run) = result
            writer.write("###BEGIN###" + "\n")

            writer.write("[RUN]\t" + current_config + "\n")
            writer.write("[FEATURES]\t" + numberOfSelectedFeatures + "\n")
            writer.write("[ASTNODES]\t" + numberOfASTElements + "\n")
            writer.write("[PRODUCTGENERATION]\t" + productDerivationTime + "\n")
            ...
            if (sum)
              ...
              timings.foreach {
                  case (analysis, time) if singleErrs.contains(analysis) =>
                    singleErrs.get(analysis).get.foreach(error => writer.write("[" + analysis.toUpperCase + "_ERRDETAIL]\t" + PrettyPrinter.print(parentOpt(error.where.asInstanceOf[AST], env).entry.asInstanceOf[AST]) + "\n"))
                  case _ =>
                }
              }
            writer.write("###END###" + "\n")
      ...
      val path =
        if (sum) opt.getFile + "_" + taskDesc + "_product.sumreport.gz"
        else opt.getFile + "_" + taskDesc + "_product.detailreport.gz"

      val f: File = new File(path)
      f.getParentFile.mkdirs()

      writeResults(gzipWriter(f)).close
    }
  }
  ...
  private def doStaticAnalysisWithSA(ast: TranslationUnit, fm: FeatureModel, opt: FamilyBasedVsSampleBasedOptions): (List[TypeChefError], List[(String, Long)], Map[String, List[TypeChefError]], CIntraAnalysisFrontendF) = {
    ...
    val err1 = Map("casetermination" -> sa.caseTermination())
    stopWatch.start("none")

    stopWatch.start("doublefree")
    val err2 = Map("doublefree" -> sa.doubleFree())
    stopWatch.start("none")

    stopWatch.start("uninitializedmemory")
    val err3 = Map("uninitializedmemory" ->sa.uninitializedMemory())
    stopWatch.start("none")

    stopWatch.start("xfree")
    val err4 = Map("xfree" -> sa.xfree())
    stopWatch.start("none")

    stopWatch.start("danglingswitchcode")
    val err5 = Map("danglingswitchcode" -> sa.danglingSwitchCode())
    stopWatch.start("none")

    stopWatch.start("cfginnonvoidfunc")
    val err6 = Map("cfginnonvoidfunc" -> sa.cfgInNonVoidFunc())
    stopWatch.start("none")

    stopWatch.start("checkstdlibfuncreturn")
    val err7 = Map("checkstdlibfuncreturn" -> sa.stdLibFuncReturn())
    stopWatch.start("none")

    stopWatch.start("deadstore")
    val err8 = Map("deadstore" -> sa.deadStore())
